...
import { UiBadge, UiButton, UiCard, UiEmptyState, UiSkeleton } from "./ui";
import { useSubscription } from "../hooks/useSubscription";
import TradeSourceBadge from "./TradeSourceBadge";
import type { TradeSource } from "../lib/tradeSource";
import { formatMtDateTime, formatPnlPercent, formatTradePrice } from "../lib/tradeFormat";
...
export default function TradeJournalPanel() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["trades"],
    queryFn: async () => (await api.get("/api/trades?page=1&pageSize=100")).data,
  });
  ...
  const createTrade = useMutation({
    ...
    onSuccess: () => {
      setCreateOpen(false);
      notifyActivity("Trade saved to your journal.", "success");
      void qc.invalidateQueries({ queryKey: ["trades"] });
      void qc.invalidateQueries({ queryKey: ["summary"] });
    },
    onError: (err) => notifyActivity(formatApiError(err, "Could not save trade."), "error"),
  });
    ...
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <UiButton variant="ghost" onClick={() => syncMt5.mutate()} disabled={syncMt5.isPending}>
                {syncMt5.isPending ? "Syncing…" : "Sync MT5 history"}
              </UiButton>
              <UiButton href="/app/settings?tab=mt" variant="ghost">
                Connect MT4/MT5
              </UiButton>
        ...
        <UiCard className="mt-4 overflow-hidden">
          <div className="flex flex-col gap-2 border-b border-slate-200 px-4 py-3 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm font-semibold text-slate-800 dark:text-slate-100">
              Trade records ({sortedTrades.length} of {totalOnRecord || items.length})
            </div>
          </div>
          {sortedTrades.length === 0 ? (
            <div className="p-4">
              <UiEmptyState
                ...
                description={
                  items.length === 0
                    ? "Connect MT5 and click Sync on Live trading, or add trades manually."
            ...
            <thead className="bg-slate-100 text-[10px] uppercase tracking-wide text-slate-500 dark:bg-white/5 dark:text-slate-400">
              <tr>
            ...
            <tbody>
              {sortedTrades.map((trade) => {
                const pnlClass = trade.pnl >= 0 ? "text-blue-600 dark:text-blue-400" : "text-rose-600 dark:text-rose-400";
                const pct = trade.pnlPercent ?? 0;
                return (
                <tr key={trade.id} className="border-t border-slate-200 hover:bg-slate-50 dark:border-white/10 dark:hover:bg-white/5">
                  <td className="p-2 tabular-nums text-slate-600 dark:text-slate-300">
                    {formatMtDateTime(trade.openTime)}
                  </td>
                  <td className="p-2 font-medium lowercase">
                    <Link to={`/app/trades/${trade.id}`} className="hover:text-blue-500">
                      {trade.symbol}
                    </Link>
                  </td>
                  <td className="p-2 tabular-nums text-slate-500">{trade.mtTicket ?? "—"}</td>
                  <td className="p-2">
                    <span className={trade.side === "LONG" ? "text-blue-600" : "text-rose-600"}>
                      {trade.side === "LONG" ? "buy" : "sell"}
                    </span>
                  </td>
                  <td className="p-2 text-right tabular-nums">{trade.lotSize}</td>
                  <td className="p-2 text-right tabular-nums">{formatTradePrice(trade.symbol, trade.entryPrice)}</td>
                  <td className="p-2 tabular-nums text-slate-600 dark:text-slate-300">
                    {formatMtDateTime(trade.closeTime)}
                  </td>
                  <td className="p-2 text-right tabular-nums">{formatTradePrice(trade.symbol, trade.exitPrice)}</td>
                  <td className="p-2">
                    <TradeSourceBadge source={trade.source} sourceLabel={trade.sourceLabel} />
                  </td>
                  <td className={`p-2 text-right font-semibold tabular-nums ${pnlClass}`}>
                    {trade.pnl >= 0 ? "" : ""}
                    {Number(trade.pnl).toFixed(2)}
                    {trade.source === "WEBSITE" && (trade.platformFee ?? 0) > 0 ? (
                      <span className="block text-[10px] font-normal opacity-70">
                        gross {(trade.grossPnl ?? trade.pnl).toFixed(2)}
                      </span>
                    ) : null}
                  </td>
                  <td className={`p-2 text-right tabular-nums ${pnlClass}`}>{formatPnlPercent(pct)}</td>
                  <td className="p-2 text-right tabular-nums text-slate-500">
                    {(trade.platformFee ?? 0) > 0 ? `-${trade.platformFee!.toFixed(2)}` : "—"}
                  </td>
